...
if [ ! -f ./maf/"${FNAME}".vep.maf ]; then
	if [ ! -f ./completed/"${FNAME}".snp.vcf ]; then
		samtools mpileup -q 1 -f "${RES}" ../bam/"${TUMOR}".bam > "${PID}".pileup
		varscan somatic "${STUDY_DIR}"/varscan/"${PID}".pileup "${STUDY_DIR}"/varscan/"${PID}" --somatic-p-value 0.05 --p-value 0.99 --output-vcf 1
