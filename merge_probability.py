...
for entry in glob.glob("*.sorted.table"):
	...
	if len( table['GENE'].unique() ) < len( table.index ):
		...
		for i in unique:
			...
			val = pd.to_numeric( tmp[ names[1] ] )
			product = 1
			for x in val:
				product = ( product * ( 1 - ( x / 100 ) ) )
			df2 = pd.DataFrame([[i, product, 1, 1]], columns=names)
			df = df.append( df2, ignore_index=True )
		df = df.sort_values( by=['GENE'] )
		table = df
		table.to_csv("test.table", sep="\t", quoting=None, index=False )
...
if os.path.exists("../Final.table3"):
