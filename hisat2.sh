...
if [ ! -f ../bam/"${FNAME}".bam ]; then
	hisat2 -p 2 --dta -x "${REF_DIR}"/GRCh38 -1 "${FNAME}"_1.fastq -2 "${FNAME}"_2.fastq -S "${FNAME}".sam --summary-file ../summary/"${FNAME}".txt
	samtools sort -@ 2 -o ../bam/"${FNAME}".bam "${FNAME}".sam
	samtools index ../bam/"${FNAME}".bam
	rm "${FNAME}".sam
fi
